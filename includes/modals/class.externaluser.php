...
class Externaluser extends DatabaseObject
{

    protected static $table_name = "    tbl_external_users";
    protected static $db_fields = array('id', 'firstname', 'lastname', 'phone', 'email', 'username', 'password', 'accesskey', 'image', 'facebook','address', 'access_code', 'website', 'sortorder', 'status', 'created_at', 'updated_at');
    ...
    // get external user id by access token
    ...
    public static function get_access_by($token = '')
    {
        ...
        $sql = "SELECT id, firstname, lastname, email, accesskey FROM " . self::$table_name . $cond;
        $result = self::find_by_sql($sql);
        return !empty($result) ? array_shift($result) : false;
    }
    ...
    {
        global $db;
        $result_array = self::find_by_sql("SELECT `id`,`email`,`firstname`,`lastname` FROM " . self::$table_name . " WHERE `email`='" . $email . "' LIMIT 1");
    ...
    // get external user email address by user_id
    ...
    {
        global $db;
        $sql = "SELECT id,firstname,lastname, email FROM " . self::$table_name . " WHERE id=" . $id;
        $result = self::find_by_sql($sql);
        return !empty($result) ? array_shift($result) : false;
    }
    
    // get the externaluser name from it's id
    static function getExternaluserName($id=0){
        global $db;
        $result = $db->query("SELECT firstname FROM ".self::$table_name." WHERE id='{$id}'");
        $return = $db->fetch_array($result);
        return ($return) ? $return['firstname'] : '' ;
    }
