...
class Review extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'name', 'comments', 'email', 'event_id', 'sortorder', 'status', 'added_date');
    ...
    {
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE event_id=$packageid AND status=1 ORDER BY sortorder DESC");
    }

    public static function find_for_homepage()
    {
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1 AND homepage=1 ORDER BY id DESC");
    }

    public static function getTotalSub($package_id = '')
    {
        global $db;
        $cond = !empty($package_id) ? ' AND event_id=' . $package_id : '';
    ...
    public static function getTotalSubApanel($package_id = '')
    {
        global $db;
        $cond = !empty($package_id) ? ' AND event_id=' . $package_id : '';
    ...
    {
        global $db;
        $result = $db->query("SELECT MAX({$field}) AS maximum FROM " . self::$table_name . " WHERE event_id={$pid}");
        $return = $db->fetch_array($result);
        return ($return) ? ($return['maximum'] + 1) : 1;
    }
