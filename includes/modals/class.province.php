...
class Province extends DatabaseObject {
	...
	protected static $table_name = "tbl_province";
	protected static $db_fields = array('id' ,'title', 'capital', 'no_of_district', 'area', 'status', 'sortorder');
	...
	// get the privience name from it's id
	static function getProvinceName($id=0){
		global $db;
	...
	//Filter all province 
    public static function get_all_provincedata($provid=0){
        global $db;     
        $sql = "SELECT id,title FROM ".self::$table_name." WHERE status='1' ORDER BY sortorder ASC";
