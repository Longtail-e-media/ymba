...
class Location extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'title', 'status', 'sortorder','type');
	...
	public static function getDistrict_limit($type=0,$limit='',$id=0){
		global $db;
	...
	static function getallDistrict($limit=''){
		global $db;
		$cond = !empty($limit)?' LIMIT '.$limit :'';
	...
	public static function getTotalloc($type=''){
		global $db;
	...
	public static function get_relatedldistrict($type='', $notid='', $limit='')
	{
		global $db;
		$cond = !empty($type)?' AND type='.$type:'';
     ...
     public static function get_all_locationdata_front($distid=0, $selid=''){
		global $db;		
		$sql = "SELECT id,title FROM ".self::$table_name." WHERE type='$distid' ORDER BY sortorder DESC";
		$record = self::find_by_sql($sql);
		$result='';
		if($record){
				// $result.='<option value="all">Search Location</option>';
...
//Filter all location
    public static function get_all_locationdata($locid=0){
        global $db;     
