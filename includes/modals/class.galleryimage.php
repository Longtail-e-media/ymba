...
class GalleryImage extends DatabaseObject
{
    ...
    public static function getImagelist_by($galleryid = '', $postnumbers = '', $offset = '')
    {
        ...
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status=1 AND galleryid=$galleryid ORDER BY sortorder DESC $limt";
    ...
    static function find_all_limit($lmt = 100)
    {
        ...
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1  AND galleryid<>7  ORDER BY sortorder ASC LIMIT $lmt");
    ...
    static function getGalleryImages($id = 0, $total = 10, $offset = 0)
    {
        global $db;
