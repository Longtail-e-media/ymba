...
function getMenuList($caption = "", $link = "", $type = 0, $class = "", $nicon = "")
{
    ...
    $href = ($type == 0) ? BASE_URL . $linkhref : $linkhref;
    return "<a href=\"" . $href . "\" " . $classLink  . $linkType . $dropclass . " >" . $caption . "</a>";
}
...
function get_front_pagination($total = '', $per_page = '2', $page = '1', $url = '')
{
    ...
    $pagination = "";
    if ($lastpage > 1) {
        $pagination .= "<div class=\"pagination mr-auto\">";
        if ($page > 1)
            $pagination .= "<a href='" . $url . '/page/' . $prev . "'>&laquo;</a>";
        else
        ...
        if ($lastpage < 7 + ($adjacents * 2)) {
            ...
            if ($page < 1 + ($adjacents * 2)) {
                for ($counter = 1; $counter < 4 + ($adjacents * 2); $counter++) {
                    if ($counter == $page)
                        $pagination .= "<a href='javascript:void(0);' class='active'>$counter</a></li>";
                    else
                        $pagination .= "<a href='" . $url . '/page/' . $counter . "'>$counter</a>";
                }
                $pagination .= "<a>...</a>";
        ...
        if ($page < $counter - 1)
            $pagination .= "<a href='" . $url . '/page/' . $next . "'>&raquo;</a>";
        else
