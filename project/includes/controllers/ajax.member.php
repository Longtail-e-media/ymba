...
//        $record->image              = serialize(array_values(array_filter($_REQUEST['imageArrayname'])));
        $record->banner             = serialize(array_values(array_filter($_REQUEST['imageArrayname1'])));
...
//        $record->linktype           = $_REQUEST['linktype'];
        ...
        $checkDupliName = Member::checkDupliName($record->title);
        if ($checkDupliName):
            echo json_encode(array("action" => "warning", "message" => "Member Title Already Exists."));
            exit;
        endif;
        $db->begin();
    ...
    case "edit":
        ...
        if ($record->title != $_REQUEST['title']) {
            $checkDupliName = Member::checkDupliName($_REQUEST['title']);
            if ($checkDupliName):
                echo json_encode(array("action" => "warning", "message" => "Member title is already exist."));
                exit;
            endif;
        }
...
//        $record->linktype           = $_REQUEST['linktype'];
    ...
    case "bulkToggleStatus":
        ...
        for ($i = 1; $i < count($allid); $i++) {
            $record = Member::find_by_id($allid[$i]);
            $record->status = ($record->status == 1) ? 0 : 1;
            $record->save();
        }
        echo "";
    ...
    case "bulkDelete":
        ...
        $db->begin();
        for ($i = 1; $i < count($allid); $i++) {
            $res = $db->query("DELETE FROM tbl_member WHERE id='" . $allid[$i] . "'");
            $return = 1;
        }
    ...
    case "sort":
        ...
        datatableReordering('tbl_member', $sortIds, "sortorder", '', '', 1);
