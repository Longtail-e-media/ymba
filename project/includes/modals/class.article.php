...
class Article extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'title', 'sub_title', 'category_id', 'linksrc', 'linktype', 'content', 'status', 'sortorder', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'homepage', 'banner', 'image');
    ...
    public static function find_articles_by_category($catId = '')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' AND category_id='$catId' ORDER BY sortorder DESC";
        return self::find_by_sql($sql);
    }
