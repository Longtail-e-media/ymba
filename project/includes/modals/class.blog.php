...
class Blog extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'title', 'author', 'brief', 'content', 'status', 'meta_keywords', 'meta_description', 'blog_date', 'thumb_upload', 'archive_date', 'sortorder', 'type', 'viewcount', 'added_date', 'image', 'source');
    ...
    public static function get_latestblog_by($limit = '')
    {
        ...
        $sql = "SELECT id, slug, title, author, brief, image, thumb_upload, blog_date, source, viewcount FROM " . self::$table_name . " WHERE status='1' AND type='0' ORDER BY id DESC $cond";
        $result = self::find_by_sql($sql);
        return $result;
    }
    
    public static function get_latestnews_by($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? 'LIMIT ' . $limit : '';
        $sql = "SELECT id, slug, title, author, brief, image, thumb_upload, blog_date, source, viewcount FROM " . self::$table_name . " WHERE status='1' AND type='1' ORDER BY id DESC $cond";
        $result = self::find_by_sql($sql);
        return $result;
    }

    public static function get_latesthomeblog_by($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? 'LIMIT ' . $limit : '';
        $sql = "SELECT id, slug, title, author, brief, image, thumb_upload, blog_date, source, viewcount FROM " . self::$table_name . " WHERE status='1' AND type='0' ORDER BY blog_date DESC $cond";
        $result = self::find_by_sql($sql);
        return $result;
    }
    ...
    public static function get_popularblog($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? 'LIMIT ' . $limit : '';
        $sql = "SELECT slug, title, author, image,thumb_upload, brief, blog_date FROM " . self::$table_name . " WHERE status='1' AND MONTH(blog_date) = MONTH(CURRENT_DATE) ORDER BY viewcount DESC $cond";
        $result = self::find_by_sql($sql);
        return $result;
    }
    ...
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " ORDER BY blog_date DESC";
