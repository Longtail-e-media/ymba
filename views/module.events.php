...
if (defined('HOME_PAGE')) {
    $events = Events::get_relatedevnt('','',3);
    if (!empty($events)) {
        $i = 0;
        $duration = 0;
        foreach ($events as $event) {
            ...
            $duration = ($i % 3 == 0) ? $duration + 1000 : $duration;
                ...
                $by = (!empty($event->published_in)) ? '<h6>Author: '.$event->published_in.'</h6>' : '' ;
                $home_events .= '
                    <div class="col-md-4 col-sm-4 col-xs-12 mar-bottom-30">
                        <div class="grid our__events">
                            <div class="grid-item">
                                ...
                                <div class="gridblog-content">
                                    ...
                                    '.$by.'
                                    <div class="event_prgms"><i class="fas fa-calendar"></i> '.date('M d, Y', strtotime($event->event_stdate)).'</div>
                                    <p>'.$event->brief.'</p>
                                   <div class="view_more">
                                  <a href="'.BASE_URL.'eventsdetail/'.$event->slug.'">Read More </a>
                                   </div>
                                </div>
                            </div>
                        </div>
        ...
        if (!empty($eventRec)) {
            ...
            $events_banner .= '
                <div class="article-banner">
                    ...
                    <span>    <i class="far fa-calendar-alt"> ' . date('dS M Y', strtotime($eventRec->event_stdate)) . '</i>
                        '.$by.'</span>
                    </div>
                </div>
            ';
