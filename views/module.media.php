...
if (defined('MEDIA_DETAIL_PAGE')) {
    if (isset($_REQUEST['slug']) and !empty($_REQUEST['slug'])) {
        ...
        if (!empty($eventRec)) {
            ...
            $events_detail .= '
                ...
                <!-- sidebar starts -->
                    <div class="col-md-4 col-sm-4 col-xs-12 pad-left-30">
                        ...
                        <a href="' . BASE_URL . 'media" class="btn btn-success">Back</a>
                ';
                ...
                foreach ($relatedEvents as $relatedEvent) {
                    if (!empty($relatedEvent->content)) {
                        ...
                        if (!empty($relatedEvent->linksrc)) {
                            ...
                            $linkstart = '<a href="' . BASE_URL . 'mediadetail/' . $relatedEvent->slug . '" class="title">';
                            $linkend = '</a>';
                        }
                        $events_detail .= '
                            <article class="post mar-bottom-20">
                                <div class="content display-flex">
                                    <div class="blog-no">
                                        <!--<img src="' . $banner_img . '" alt="' . $relatedEvent->title . '">-->
                                        0'.$i.'
                                    </div>
            ...
            $sql = "SELECT (event_stdate) FROM tbl_events WHERE status='1' AND type=2 GROUP BY YEAR(event_stdate) ORDER BY YEAR(event_stdate) DESC";
            ...
            if (!empty($archives)) {
                ...
                foreach ($archives as $archive) {
                    ...
                    $events_detail .= '
                        <li><a href="' . BASE_URL . 'media/' . $year . '/page/1">' . $year . '</a></li>
                    ';
                }
                $events_detail .= '
                        </ul>
...
$jVars['module:media-banner'] = $events_banner;
$jVars['module:media-detail'] = $events_detail;
...
if (defined('MEDIA_PAGE')) {
    ...
    if (!empty($search_term)) {
        $sql = "SELECT * FROM tbl_events WHERE status='1' AND type=2 AND (
            ...
            foreach ($Records as $record) {
                ...
                if (file_exists($file_path) and !empty(@$images[0])) {
                    ...
                    if (!empty($record->linksrc)) {
                        ...
                        $linkstart = (!empty($record->content)) ? '<a href="' . BASE_URL . 'mediadetail/' . $record->slug . '">' : '';
                                ...
                                <div class="grid-item">
                                    ...
                                    <div class="gridblog-content">
                                        <div class="date mar-bottom-15 d-flex justify-content-between">
                                            ...
                                            <!--<div>'.$comments_number.' '.$comments.'</div>-->
                                            <a href="'.BASE_URL.'mediadetail/'.$record->slug.'#review_form" style="text-decoration: underline;color: #3aaa36;">'.$comments_number.' '.$comments.'</a>
                                        </div>
        ...
        if (!empty($year)) {
            $sql = "SELECT * FROM tbl_events WHERE status='1' AND type=2 AND YEAR(event_stdate) = " . $year . " ORDER BY event_stdate DESC";
        } else {
            $sql = "SELECT * FROM tbl_events WHERE status='1' AND type=2 ORDER BY event_stdate DESC";
        }
        $limit = 9;
        ...
        if (!empty($Records)) {
            ...
            // Pagination
            if (!empty($year)) {
                $events_list_pagination .= get_front_pagination($total, $limit, $page, BASE_URL . 'media/' . $year);;
            } else {
                $events_list_pagination .= get_front_pagination($total, $limit, $page, BASE_URL . 'media');
            }
        }
    }

}
$jVars['module:media-list-banner'] = $events_list_banner;
$jVars['module:media-list-detail'] = $events_list_detail;
$jVars['module:media-list-pagination'] = $events_list_pagination;